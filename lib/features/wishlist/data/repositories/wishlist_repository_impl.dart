...
class WishlistRepositoryImpl extends FirestoreBaseRepository
    implements WishlistRepository {
  WishlistRepositoryImpl(super.firestore);

  @override
  Stream<Either<Failure, List<WishlistItemModel>>> watchWishlist(
      String userId) {
    return safeFirestoreStream(
      () => firestore
          ...
          .snapshots()
          .map((snap) =>
              snap.docs.map(WishlistItemModel.fromFirestore).toList()),
    );
  }

  @override
  Future<Either<Failure, void>> addToWishlist(
      String userId, WishlistItemModel item) {
    return safeFirestoreCall(() async {
      ...
      final wishlistRef = firestore
          ...
          .doc(item.productId);
      final userRef =
          firestore.collection(FirestoreConstants.users).doc(userId);
